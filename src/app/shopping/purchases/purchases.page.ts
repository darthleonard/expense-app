...
export class PurchasesPage implements OnInit {
  ...
  openEditModal(purchase: Purchase) {
    ...
    this.newPurchase = {
      ...purchase,
      isPlanned: purchase.isPlanned ?? false,
      purchaseDate: purchase.purchaseDate
        ? purchase.purchaseDate.substring(0, 10)
        : purchase.creationDate.substring(0, 10),
    };
    this.isModalOpen = true;
  }
  ...
  async savePurchase() {
    ...
    const est = this.establishments.find(
      (e) => e.id === this.newPurchase.establishmentId
    );
      ...
      if (this.editingPurchase) {
        const updated: Purchase = {
          ...
          purchaseDate: this.newPurchase.purchaseDate
            ? new Date(this.newPurchase.purchaseDate).toISOString()
            : undefined,
          isPlanned: this.newPurchase.isPlanned ?? false,
        };
